...
draw_rect (Window win, XRectangle *r)
{
    ...
    /* 创建一个宽度与damage区域相同的pixmap */
    pixmap  = XCreatePixmap (dpy, win, r->width, r->height, 24);
    /* 填充制定位置，尺寸的gc到pixmap */
    XFillRectangle (dpy, pixmap, gc, r->x, r->y, r->width, r->height);
    ...
    /* 创建源picture */
    Picture picture_src;
    picture_src = XRenderCreatePicture (dpy, watchWindow, pictureFormat, 0, NULL);
    /* void
       XRenderComposite (Display    *dpy,
                         ...
                         unsigned int   height);
     */
    /* 从damage起始位置拷贝damage区域到目标 */
    XRenderComposite (dpy, PictOpOver, picture_src, None, picture, r->x, r->y, 0, 0, 0, 0, r->width, r->height);
#else
    color.red   = 0x0000;
    color.green = 0x0000;
    color.blue  = 0xff00;
    ...
    XCopyArea (dpy, pixmap, win, gc, 0, 0, r->width, r->height, r->x, r->y);
